...
FORECAST_MONTHS = 12   # how many months ahead to predict (after full series)
TEST_MONTHS     = 12   # hold out last N months to evaluate out-of-sample MAPE
...
def plot_diagnostics(series: pd.Series, crime: str) -> None:
    ...
    plot_acf( series.diff().dropna(), lags=36, ax=axes[0], title=f"ACF — {crime}")
    plot_pacf(series.diff().dropna(), lags=36, ax=axes[1], title=f"PACF — {crime}")
    plt.tight_layout()
    plt.show()
...
def fit_and_forecast(series: pd.Series, crime: str) -> None:
    ...
    ax.set_title(f"{order_str} — Train/Test Evaluation: {crime}", fontsize=13, fontweight="bold")
    ax.set_xlabel("Date")
    ...
    # Refit on full series, then forecast into the future
    print("\n  Refitting on full series for future forecast …")
    auto_model.update(test)   # extend the model with the test observations
    ...
    # Plot full series + future forecast
    fig, ax = plt.subplots(figsize=(14, 5))

    ax.plot(series.index, series, label="Observed (full)", color="#2c7bb6", linewidth=1.8)
    ...
    ax.set_title(f"{order_str} — Future Forecast: {crime}", fontsize=13, fontweight="bold")
    ax.set_xlabel("Date")
    ax.set_ylabel("Number of incidents")
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%b %Y"))
    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=6))
